...
class MyBinaryTree : public BinaryTree {
    ...
    void zigleft(node* curr) { //curr is 8
		node* par = curr->parent; // 5
		node* gpar = par->parent; // ? might be null
		node* shared = curr->left; // 7
		
		curr->left = par;
		par->parent = curr;
		
		par->right = shared;
		if (shared != nullptr) shared->parent = par;
		curr->parent = gpar;
    ...
    void zigright(node* curr) {//curr is 8
		node* par = curr->parent; // 5
		node* gpar = par->parent; // ? might be null
		node* shared = curr->right; // 7
